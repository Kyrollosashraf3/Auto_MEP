...
<body>
    ...
    <div class="main">
      ...
      <div class="content" id="content">
        <div class="empty-state">
          ...
          <p>Pick a project from the sidebar, or create a new one to start uploading room data and running load
            analysis.</p>
        </div>
      </div>
    </div>
  ...
  <div class="modal-overlay" id="newProjectModal">
    <div class="modal-box framed">
      <div class="modal-head">
        <h3>New Project</h3><button class="modal-close" onclick="closeModal('newProjectModal')">&times;</button>
      </div>
  ...
  <div class="modal-overlay" id="editProjectModal">
    <div class="modal-box framed">
      <div class="modal-head">
        <h3>Edit Project</h3><button class="modal-close" onclick="closeModal('editProjectModal')">&times;</button>
      </div>
  ...
  <div class="modal-overlay" id="resultModal">
    <div class="modal-box wide framed">
      <div class="modal-head">
        <h3 id="resultTitle">Result</h3><button class="modal-close" onclick="closeModal('resultModal')">&times;</button>
      </div>
      <div class="modal-body" id="resultBody"></div>
      <div class="modal-foot">
        <button class="btn btn-ghost btn-sm" id="resultDownloadBtn" onclick="downloadCurrentResult()" disabled>Download
          PDF</button>
      </div>
    </div>
  </div>
  ...
  `;
    ...
    async function downloadCurrentResult() {
      ...
      try {
        const endpoint = lastResult.type === 'analysis'
          ? '/analysis/' + lastResult.fileId + '/download'
          : '/analysis/generate_report/' + lastResult.fileId + '/download';
        ...
        a.href = url;
        a.download = (lastResult.type === 'analysis' ? 'analysis_' : 'report_') + sanitizeName(lastResult.fileName) + '.pdf';
    ...
    `;
        lastResult = { type: 'analysis', fileId: fileId, fileName: fileName };
    ...
    async function runReport(fileId, fileName) {
      ...
      try {
        ...
        lastResult = { type: 'report', fileId: fileId, fileName: fileName };
        document.getElementById('resultDownloadBtn').disabled = false;
      } catch (e) {
        document.getElementById('resultBody').innerHTML = '<div class="auth-error">' + escapeHtml(e.message) + '</div>';
      }
